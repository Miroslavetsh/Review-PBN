...
try {
    getBonusesFrom('../others/data.json', 10)

    async function getBonusesFrom(url, bonusesToShow) {
        let response = await fetch(url)
        let bonusesArray = await response.json()
        let slicedbonusesArray = bonusesArray.slice(0, bonusesToShow)

        slicedbonusesArray.forEach((bonus) => {
            insertBonusCard(bonus.elem)
        })
        ;(await bonusesArray) ? additionalInfoShow() : pass
    }

    function makeBonusCard(element) {
                    ...
                    <div class="bonus__visible">
                        <div class="bonus__img">
                            ...
                            <div class="bonus__text">
                                ...
                                <span class="bonus__bonus">Фриспины: 
                                    <strong>
                                    ${
                                        element.casinoBonus.indexOf('FS') !== -1
                                            ? element.casinoBonus
                                            : element.casinoBonus + ' FS'
                                    }
                                    </strong>
                                </span>
                            </div>
                        ...
                        <div class="bonus__buttons">
                            <a href="#" class="button button--purple">Взять бонус</a>
    ...
    function insertBonusCard(card) {
        let bonuses = document.querySelector('.bonuses')
        let wrapperToInsertBonuses = bonuses.querySelector('.bonuses__bonuses')
        let bonusCard = makeBonusCard(card)

        wrapperToInsertBonuses.insertAdjacentHTML('beforeend', bonusCard)
    }

    // Additional info Button

    async function additionalInfoShow() {
